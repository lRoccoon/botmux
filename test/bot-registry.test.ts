...
describe('loadBotConfigs', () => {
  ...
  it('defaults requireApproval to false when omitted', () => {
    process.env.BOTS_CONFIG = '/tmp/no-require-approval.json';
    fsMock.existsSync.mockReturnValue(true);
    fsMock.readFileSync.mockReturnValue(JSON.stringify([{
      larkAppId: 'app_default',
      larkAppSecret: 'secret_default',
    }]));

    const configs = mod.loadBotConfigs();
    expect(configs[0].requireApproval).toBe(false);
  });
