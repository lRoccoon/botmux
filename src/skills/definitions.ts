...
- 派活靠 \`botmux dispatch\`：在当前话题会话里调用时，默认**追加到当前话题**并 @ 子 bot；在普通群 chat-scope 会话里调用时才会发种子消息开新子话题。**子 bot 必须已在群里且 mentionable**（有 include_bot 权限）。
...
\`\`\`
如果当前主编排会话本身就在一个飞书话题里，上面的命令会默认把派活消息追加到**当前话题**；要追加到别的已存在话题，用 \`--into <话题root>\`。
