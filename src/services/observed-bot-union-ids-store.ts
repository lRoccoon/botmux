 ...
 *
 * Storage: `{dataDir}/observed-bot-union-ids.json`, atomic writes (unique tmp + rename).
 *
 * NB: distinct from services/bot-union-ids-store.ts (master), which stores each
 * LOCAL bot's OWN union_id keyed by larkAppId for the platform heartbeat. This
 * store is the verified-delivery/federation registry keyed by botName, learned
 * by observing OTHER bots' sender events — different shape, different consumers,
 * so it lives in its own file to avoid clobbering the platform store.
 */
