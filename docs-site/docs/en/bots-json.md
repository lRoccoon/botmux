...
| `showUsageInCardFooter` | Whether reply-card footers show native Context / Token usage from the Agent CLI. Missing / `true` = show; `false` = hide both metrics. A missing individual metric is still omitted independently. This controls card display only and does not disable the Usage Ledger or other accounting. For claude-code the Claude Code statusline is bridged in: when data is present the context segment becomes a bare percentage `ctx 23%` and account-level quota `5h 18% · 7d 5%` is appended (omitted when absent, never inferred) |
